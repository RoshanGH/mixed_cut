...
struct RewriteResultMapperTests {
    // 预算按原台词字数（forOriginalLength，下限=原字数，上限 1.15×）：
    // s1 = 13 字 → 区间 13~15
    // s2 = 9 字  → 区间 9~10
    ...
    func fullCoverage() {
        let dto = RewriteResultDTO(segments: [
            .init(segmentId: "s2", rewrittenText: String(repeating: "乙", count: 9)),   // 9 字 → 9~10 内
            .init(segmentId: "s1", rewrittenText: String(repeating: "甲", count: 13))   // 13 字 → 13~15 内
        ])
    ...
    func extraIgnoredAndBudget() {
        let dto = RewriteResultDTO(segments: [
            .init(segmentId: "s1", rewrittenText: String(repeating: "丙", count: 16)), // 16 字 → >15 超上限
            .init(segmentId: "s2", rewrittenText: String(repeating: "乙", count: 9)),
